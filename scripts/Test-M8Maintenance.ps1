...
try {
    ...
    if ($health.databaseSchemaVersion -ne 15 -or -not $health.connectorConnected) {
        throw 'Empty production startup did not reach Core schema 15 with Connector ready.'
    }
...
'@
    ...
    if ($firstMigration.migrated -or $secondMigration.migrated -or
        $secondMigration.coreSchemaVersion -ne 15 -or
        $secondMigration.connectorSchemaVersion -ne 3) {
        throw 'Repeated migration was not idempotent at Core 15 / Connector 3.'
    }
