...
$coreHost = if ([string]::IsNullOrWhiteSpace($env:AICL_CORE_HOST)) {
    '127.0.0.1'
} else {
    $env:AICL_CORE_HOST
}
$env:AICL_CORE_HOST = $coreHost
$corePort = if ([string]::IsNullOrWhiteSpace($env:AICL_CORE_PORT)) {
    '8787'
} else {
    $env:AICL_CORE_PORT
}
$env:AICL_CORE_PORT = $corePort
if ([string]::IsNullOrWhiteSpace($env:VITE_CORE_WS_URL)) {
    $webSocketHost = if ($coreHost -eq '::1') { '[::1]' } else { $coreHost }
    $env:VITE_CORE_WS_URL = "ws://${webSocketHost}:$corePort/ws"
}
