...
try {
    ...
    if ($health.databaseSchemaVersion -ne 9 -or -not $health.connectorConnected) {
        throw 'Empty production startup did not reach Core schema 9 with Connector ready.'
    }
...
'@
    ...
    if ($firstMigration.migrated -or $secondMigration.migrated -or
        $secondMigration.coreSchemaVersion -ne 9 -or
        $secondMigration.connectorSchemaVersion -ne 3) {
        throw 'Repeated migration was not idempotent at Core 9 / Connector 3.'
    }
