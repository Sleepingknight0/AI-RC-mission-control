...
try {
    ...
    if ($health.databaseSchemaVersion -ne 16 -or -not $health.connectorConnected) {
        throw 'Empty production startup did not reach Core schema 16 with Connector ready.'
    }
...
'@
    ...
    if ($firstMigration.migrated -or $secondMigration.migrated -or
        $secondMigration.coreSchemaVersion -ne 16 -or
        $secondMigration.connectorSchemaVersion -ne 3) {
        throw 'Repeated migration was not idempotent at Core 16 / Connector 3.'
    }
