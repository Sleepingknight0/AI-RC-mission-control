...
try {
    ...
    if ($health.databaseSchemaVersion -ne 7 -or -not $health.connectorConnected) {
        throw 'Empty production startup did not reach Core schema 7 with Connector ready.'
    }
...
'@
    ...
    if ($firstMigration.migrated -or $secondMigration.migrated -or
        $secondMigration.coreSchemaVersion -ne 7 -or
