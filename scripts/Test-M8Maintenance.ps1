...
try {
    ...
    if ($health.databaseSchemaVersion -ne 10 -or -not $health.connectorConnected) {
        throw 'Empty production startup did not reach Core schema 10 with Connector ready.'
    }
...
'@
    ...
    if ($firstMigration.migrated -or $secondMigration.migrated -or
        $secondMigration.coreSchemaVersion -ne 10 -or
        $secondMigration.connectorSchemaVersion -ne 3) {
        throw 'Repeated migration was not idempotent at Core 10 / Connector 3.'
    }
