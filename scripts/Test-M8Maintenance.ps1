...
try {
    ...
    if ($health.databaseSchemaVersion -ne 17 -or -not $health.connectorConnected) {
        throw 'Empty production startup did not reach Core schema 17 with Connector ready.'
    }
...
'@
    ...
    if ($firstMigration.migrated -or $secondMigration.migrated -or
        $secondMigration.coreSchemaVersion -ne 17 -or
