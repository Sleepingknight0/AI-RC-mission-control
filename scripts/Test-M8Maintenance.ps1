...
try {
    ...
    if ($health.databaseSchemaVersion -ne 14 -or -not $health.connectorConnected) {
        throw 'Empty production startup did not reach Core schema 14 with Connector ready.'
    }
...
'@
    ...
    if ($firstMigration.migrated -or $secondMigration.migrated -or
        $secondMigration.coreSchemaVersion -ne 14 -or
        $secondMigration.connectorSchemaVersion -ne 3) {
        throw 'Repeated migration was not idempotent at Core 14 / Connector 3.'
    }
