...
- `f6e56a4` — revoke every unproven pre-v13 write grant, apply exact per-Session
  control validation to Catalog rows, and consume duplicate approval evidence
  without stranding the Connector journal.
- `6c66ceb` — align compiled Host lifecycle assertions with Core schema 13.
- `708e623` — classify every rejected durable Connector event through stable,
  structured, boot-bounded diagnostics while preserving receipt acknowledgement.

## Finding register

| Finding | Decision | Reproduction and remediation | Regression evidence | Remaining risk / Grok |
|---|---|---|---|---|
| C-M9-01 unsafe sandbox default | Accepted, High | Migration 006/default and null-project translation reproduced write-capable defaults without durable proof of operator choice, canonical root, or current capability. Schema 12 made new/default and ambiguous rows `read_only`/`denied`; schema 13 unconditionally revokes every pre-v13 `workspace_write` grant, including ready/non-null rows. Connector maps null-project workspace write to provider read-only. (`7cc955a`, `02af515`, `f6e56a4`) | `database-contract`: schema-13 fresh/idempotent and legacy upgrades, including a formerly ready write row; `codex-adapter`: null-project workspace write | None known; unblocks Grok. |
| C-M9-02 `turn.submit` creation bypass | Accepted, High | Unknown Session submission and optimistic catalog authority reproduced. Submit/subscribe no longer create rows; first Turn requires an existing ready binding and fresh exact provider/account/project/model/capability authority. Catalog `canControl` now invokes that same exact validator rather than fleet/account-only evidence. (`02af515`, `f6e56a4`) | `session-lifecycle`: unknown Session, invalid provider/account/model, inventory-only provider, validated first Turn; migrated WebSocket fixtures create explicitly; `provider-inventory-relay`: fresh removal of the bound model makes Catalog control false | None known; unblocks Grok. |
...
| C-M9-04 unvalidated sends / journal stranding | Accepted, High | Core sends lacked the boundary validator and permanently invalid durable evidence could throw before acknowledgement. Every Core-to-Connector command now parses through the strict schema. Invalid activity, file-change, artifact, approval, interrupt, terminal-Turn, and Runtime evidence is consumed, acknowledged, and classified through a stable structured callback; duplicate approval IDs also commit the source receipt. Server diagnostics are bounded to 64 plus one limit notice per boot. (`02af515`, `f6e56a4`, `708e623`) | `security-boundaries`: malformed outbound command rejected; `database-contract`: rejection-code matrix, artifact cleanup, duplicate approval poison receipt, and duplicate receipt consumption | Rejected provider evidence is intentionally absent from the authoritative timeline but remains observable through bounded diagnostics; unblocks Grok. |
...
The final local gate passed frozen installation, backup-backed schema 12→13
migration, repeat no-op migration, build, strict typecheck/lint, 185 automated tests, compiled production lifecycle,
backup/restore/restart/corruption, clean-directory install, private-Serve smoke,
and `git diff --check`. Core is schema 13 and Connector is schema 3.

The unchanged Real Codex test passed with a 71.228-second test body (Vitest
72.16 seconds; command wall 76.4 seconds). It covered explicit authoritative
...
The `code-review` Standards and Spec agents independently reviewed the complete
`b6cd015..708e623` range after the final forward fixes. Both returned PASS: the
three initially identified High gaps (unproven migrated write grants,
fleet-only Catalog control, and duplicate-approval journal stranding) are
closed, and all rejected durable event families now produce stable bounded
diagnostics. Two Low maintainability observations remain around capability-
projection module size and capability-policy scattering. They have no observed
authority, security, or behavioral effect and do not block Grok integration.
